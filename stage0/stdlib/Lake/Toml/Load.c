// Lean compiler output
// Module: Lake.Toml.Load
// Imports: public import Lean.Parser.Types public import Lake.Toml.Data.Value import Lake.Toml.Elab import Lake.Util.Message import Std.Do
#include <lean/lean.h>
#if defined(__clang__)
#pragma clang diagnostic ignored "-Wunused-parameter"
#pragma clang diagnostic ignored "-Wunused-label"
#elif defined(__GNUC__) && !defined(__CLANG__)
#pragma GCC diagnostic ignored "-Wunused-parameter"
#pragma GCC diagnostic ignored "-Wunused-label"
#pragma GCC diagnostic ignored "-Wunused-but-set-variable"
#endif
#ifdef __cplusplus
extern "C" {
#endif
lean_object* l_Std_DTreeMap_Internal_Impl_Const_get_x3f___at___00Lean_NameMap_find_x3f_spec__0___redArg(lean_object*, lean_object*);
lean_object* l_Lean_PersistentHashMap_mkEmptyEntriesArray___redArg();
lean_object* l_Lean_stringToMessageData(lean_object*);
lean_object* lean_mk_empty_array_with_capacity(lean_object*);
lean_object* l_unsafeCast___redArg(lean_object*);
lean_object* l_Lean_Name_mkStr1(lean_object*);
extern lean_object* l_Lean_firstFrontendMacroScope;
lean_object* lean_nat_add(lean_object*, lean_object*);
extern lean_object* l_Lean_maxRecDepth;
extern lean_object* l_Lean_Options_empty;
lean_object* l_Lean_mkEmptyEnvironment(uint32_t);
extern lean_object* l_Lake_Toml_toml;
lean_object* l_Lean_Data_Trie_empty___redArg();
lean_object* l_Lean_Parser_mkParserState(lean_object*);
lean_object* l_Lean_Parser_ParserFn_run(lean_object*, lean_object*, lean_object*, lean_object*, lean_object*);
lean_object* l_Lake_mkParserErrorMessage(lean_object*, lean_object*, lean_object*);
extern lean_object* l_Lean_MessageLog_empty;
lean_object* l_Lean_MessageLog_add(lean_object*, lean_object*);
uint8_t l_Lean_Parser_InputContext_atEnd(lean_object*, lean_object*);
lean_object* l_Lean_Parser_SyntaxStack_back(lean_object*);
lean_object* l_Lean_Core_getMaxHeartbeats(lean_object*);
extern lean_object* l_Lean_NameSet_empty;
lean_object* lean_st_mk_ref(lean_object*);
extern lean_object* l_Lean_inheritedTraceOptions;
lean_object* lean_st_ref_get(lean_object*);
extern lean_object* l_Lean_diagnostics;
lean_object* l_Lake_Toml_elabToml(lean_object*, lean_object*, lean_object*);
uint8_t l_Lean_MessageLog_hasErrors(lean_object*);
lean_object* l_Lake_mkExceptionMessage(lean_object*, lean_object*);
lean_object* lean_st_ref_take(lean_object*);
lean_object* l_Lean_Kernel_enableDiag(lean_object*, uint8_t);
lean_object* lean_st_ref_put(lean_object*, lean_object*);
uint8_t l_Lean_Kernel_isDiagnosticsEnabled(lean_object*);
lean_object* lean_io_error_to_string(lean_object*);
lean_object* l_Lean_MessageData_ofFormat(lean_object*);
lean_object* l_Lake_mkMessageNoPos(lean_object*, lean_object*, uint8_t);
LEAN_EXPORT uint8_t l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0(lean_object*, lean_object*);
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0___boxed(lean_object*, lean_object*);
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1(lean_object*, lean_object*);
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1___boxed(lean_object*, lean_object*);
static lean_once_cell_t l_Lake_Toml_loadToml___closed__0_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__0;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__1_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__1;
static const lean_string_object l_Lake_Toml_loadToml___closed__2_value = {.m_header = {.m_rc = 0, .m_cs_sz = 0, .m_other = 0, .m_tag = 249}, .m_size = 1, .m_capacity = 1, .m_length = 0, .m_data = ""};
static const lean_object* l_Lake_Toml_loadToml___closed__2 = (const lean_object*)&l_Lake_Toml_loadToml___closed__2_value;
static const lean_string_object l_Lake_Toml_loadToml___closed__3_value = {.m_header = {.m_rc = 0, .m_cs_sz = 0, .m_other = 0, .m_tag = 249}, .m_size = 13, .m_capacity = 13, .m_length = 12, .m_data = "end of input"};
static const lean_object* l_Lake_Toml_loadToml___closed__3 = (const lean_object*)&l_Lake_Toml_loadToml___closed__3_value;
static const lean_ctor_object l_Lake_Toml_loadToml___closed__4_value = {.m_header = {.m_rc = 0, .m_cs_sz = sizeof(lean_ctor_object) + sizeof(void*)*2 + 0, .m_other = 2, .m_tag = 1}, .m_objs = {((lean_object*)&l_Lake_Toml_loadToml___closed__3_value),((lean_object*)(((size_t)(0) << 1) | 1))}};
static const lean_object* l_Lake_Toml_loadToml___closed__4 = (const lean_object*)&l_Lake_Toml_loadToml___closed__4_value;
static const lean_ctor_object l_Lake_Toml_loadToml___closed__5_value = {.m_header = {.m_rc = 0, .m_cs_sz = sizeof(lean_ctor_object) + sizeof(void*)*3 + 0, .m_other = 3, .m_tag = 0}, .m_objs = {((lean_object*)(((size_t)(0) << 1) | 1)),((lean_object*)&l_Lake_Toml_loadToml___closed__2_value),((lean_object*)&l_Lake_Toml_loadToml___closed__4_value)}};
static const lean_object* l_Lake_Toml_loadToml___closed__5 = (const lean_object*)&l_Lake_Toml_loadToml___closed__5_value;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__6_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__6;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__7_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__7;
static const lean_string_object l_Lake_Toml_loadToml___closed__8_value = {.m_header = {.m_rc = 0, .m_cs_sz = 0, .m_other = 0, .m_tag = 249}, .m_size = 6, .m_capacity = 6, .m_length = 5, .m_data = "_uniq"};
static const lean_object* l_Lake_Toml_loadToml___closed__8 = (const lean_object*)&l_Lake_Toml_loadToml___closed__8_value;
static const lean_ctor_object l_Lake_Toml_loadToml___closed__9_value = {.m_header = {.m_rc = 0, .m_cs_sz = sizeof(lean_ctor_object) + sizeof(void*)*2 + 8, .m_other = 2, .m_tag = 1}, .m_objs = {((lean_object*)(((size_t)(0) << 1) | 1)),((lean_object*)&l_Lake_Toml_loadToml___closed__8_value),LEAN_SCALAR_PTR_LITERAL(237, 141, 162, 170, 202, 74, 55, 55)}};
static const lean_object* l_Lake_Toml_loadToml___closed__9 = (const lean_object*)&l_Lake_Toml_loadToml___closed__9_value;
static const lean_ctor_object l_Lake_Toml_loadToml___closed__10_value = {.m_header = {.m_rc = 0, .m_cs_sz = sizeof(lean_ctor_object) + sizeof(void*)*2 + 0, .m_other = 2, .m_tag = 0}, .m_objs = {((lean_object*)&l_Lake_Toml_loadToml___closed__9_value),((lean_object*)(((size_t)(1) << 1) | 1))}};
static const lean_object* l_Lake_Toml_loadToml___closed__10 = (const lean_object*)&l_Lake_Toml_loadToml___closed__10_value;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__11_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__11;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__12_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__12;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__13_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__13;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__14_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__14;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__15_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__15;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__16_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__16;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__17_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__17;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__18_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__18;
static const lean_array_object l_Lake_Toml_loadToml___closed__19_value = {.m_header = {.m_rc = 0, .m_cs_sz = sizeof(lean_array_object) + sizeof(void*)*0, .m_other = 0, .m_tag = 246}, .m_size = 0, .m_capacity = 0, .m_data = {}};
static const lean_object* l_Lake_Toml_loadToml___closed__19 = (const lean_object*)&l_Lake_Toml_loadToml___closed__19_value;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__20_once = LEAN_ONCE_CELL_INITIALIZER;
static uint8_t l_Lake_Toml_loadToml___closed__20;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__21_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__21;
static const lean_string_object l_Lake_Toml_loadToml___closed__22_value = {.m_header = {.m_rc = 0, .m_cs_sz = 0, .m_other = 0, .m_tag = 249}, .m_size = 40, .m_capacity = 40, .m_length = 39, .m_data = "failed to initialize TOML environment: "};
static const lean_object* l_Lake_Toml_loadToml___closed__22 = (const lean_object*)&l_Lake_Toml_loadToml___closed__22_value;
static lean_once_cell_t l_Lake_Toml_loadToml___closed__23_once = LEAN_ONCE_CELL_INITIALIZER;
static lean_object* l_Lake_Toml_loadToml___closed__23;
LEAN_EXPORT lean_object* l_Lake_Toml_loadToml(lean_object*);
LEAN_EXPORT lean_object* l_Lake_Toml_loadToml___boxed(lean_object*, lean_object*);
LEAN_EXPORT uint8_t l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0(lean_object* v_opts_1_, lean_object* v_opt_2_){
_start:
{
lean_object* v_name_3_; lean_object* v_defValue_4_; lean_object* v_map_5_; lean_object* v___x_6_; 
v_name_3_ = lean_ctor_get(v_opt_2_, 0);
v_defValue_4_ = lean_ctor_get(v_opt_2_, 1);
v_map_5_ = lean_ctor_get(v_opts_1_, 0);
v___x_6_ = l_Std_DTreeMap_Internal_Impl_Const_get_x3f___at___00Lean_NameMap_find_x3f_spec__0___redArg(v_map_5_, v_name_3_);
if (lean_obj_tag(v___x_6_) == 0)
{
uint8_t v___x_7_; 
v___x_7_ = lean_unbox(v_defValue_4_);
return v___x_7_;
}
else
{
lean_object* v_val_8_; 
v_val_8_ = lean_ctor_get(v___x_6_, 0);
lean_inc(v_val_8_);
lean_dec_ref_known(v___x_6_, 1);
if (lean_obj_tag(v_val_8_) == 1)
{
uint8_t v_v_9_; 
v_v_9_ = lean_ctor_get_uint8(v_val_8_, 0);
lean_dec_ref_known(v_val_8_, 0);
return v_v_9_;
}
else
{
uint8_t v___x_10_; 
lean_dec(v_val_8_);
v___x_10_ = lean_unbox(v_defValue_4_);
return v___x_10_;
}
}
}
}
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0___boxed(lean_object* v_opts_11_, lean_object* v_opt_12_){
_start:
{
uint8_t v_res_13_; lean_object* v_r_14_; 
v_res_13_ = l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0(v_opts_11_, v_opt_12_);
lean_dec_ref(v_opt_12_);
lean_dec_ref(v_opts_11_);
v_r_14_ = lean_box(v_res_13_);
return v_r_14_;
}
}
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1(lean_object* v_opts_15_, lean_object* v_opt_16_){
_start:
{
lean_object* v_name_17_; lean_object* v_defValue_18_; lean_object* v_map_19_; lean_object* v___x_20_; 
v_name_17_ = lean_ctor_get(v_opt_16_, 0);
v_defValue_18_ = lean_ctor_get(v_opt_16_, 1);
v_map_19_ = lean_ctor_get(v_opts_15_, 0);
v___x_20_ = l_Std_DTreeMap_Internal_Impl_Const_get_x3f___at___00Lean_NameMap_find_x3f_spec__0___redArg(v_map_19_, v_name_17_);
if (lean_obj_tag(v___x_20_) == 0)
{
lean_inc(v_defValue_18_);
return v_defValue_18_;
}
else
{
lean_object* v_val_21_; 
v_val_21_ = lean_ctor_get(v___x_20_, 0);
lean_inc(v_val_21_);
lean_dec_ref_known(v___x_20_, 1);
if (lean_obj_tag(v_val_21_) == 3)
{
lean_object* v_v_22_; 
v_v_22_ = lean_ctor_get(v_val_21_, 0);
lean_inc(v_v_22_);
lean_dec_ref_known(v_val_21_, 1);
return v_v_22_;
}
else
{
lean_dec(v_val_21_);
lean_inc(v_defValue_18_);
return v_defValue_18_;
}
}
}
}
LEAN_EXPORT lean_object* l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1___boxed(lean_object* v_opts_23_, lean_object* v_opt_24_){
_start:
{
lean_object* v_res_25_; 
v_res_25_ = l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1(v_opts_23_, v_opt_24_);
lean_dec_ref(v_opt_24_);
lean_dec_ref(v_opts_23_);
return v_res_25_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__0(void){
_start:
{
lean_object* v___x_26_; lean_object* v___x_27_; 
v___x_26_ = lean_box(0);
v___x_27_ = l_unsafeCast___redArg(v___x_26_);
return v___x_27_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__1(void){
_start:
{
lean_object* v___x_28_; 
v___x_28_ = l_Lean_Data_Trie_empty___redArg();
return v___x_28_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__6(void){
_start:
{
lean_object* v___x_38_; lean_object* v___x_39_; 
v___x_38_ = l_Lean_Options_empty;
v___x_39_ = l_Lean_Core_getMaxHeartbeats(v___x_38_);
return v___x_39_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__7(void){
_start:
{
lean_object* v___x_40_; lean_object* v___x_41_; lean_object* v___x_42_; 
v___x_40_ = lean_unsigned_to_nat(1u);
v___x_41_ = l_Lean_firstFrontendMacroScope;
v___x_42_ = lean_nat_add(v___x_41_, v___x_40_);
return v___x_42_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__11(void){
_start:
{
lean_object* v___x_49_; lean_object* v___x_50_; lean_object* v___x_51_; lean_object* v___x_52_; 
v___x_49_ = lean_box(0);
v___x_50_ = lean_unsigned_to_nat(1u);
v___x_51_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__0, &l_Lake_Toml_loadToml___closed__0_once, _init_l_Lake_Toml_loadToml___closed__0);
v___x_52_ = lean_alloc_ctor(0, 3, 0);
lean_ctor_set(v___x_52_, 0, v___x_51_);
lean_ctor_set(v___x_52_, 1, v___x_50_);
lean_ctor_set(v___x_52_, 2, v___x_49_);
return v___x_52_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__12(void){
_start:
{
lean_object* v___x_53_; lean_object* v___x_54_; lean_object* v___x_55_; 
v___x_53_ = lean_unsigned_to_nat(32u);
v___x_54_ = lean_mk_empty_array_with_capacity(v___x_53_);
v___x_55_ = lean_alloc_ctor(0, 1, 0);
lean_ctor_set(v___x_55_, 0, v___x_54_);
return v___x_55_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__13(void){
_start:
{
size_t v___x_56_; lean_object* v___x_57_; lean_object* v___x_58_; lean_object* v___x_59_; lean_object* v___x_60_; lean_object* v___x_61_; 
v___x_56_ = ((size_t)5ULL);
v___x_57_ = lean_unsigned_to_nat(0u);
v___x_58_ = lean_unsigned_to_nat(32u);
v___x_59_ = lean_mk_empty_array_with_capacity(v___x_58_);
v___x_60_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__12, &l_Lake_Toml_loadToml___closed__12_once, _init_l_Lake_Toml_loadToml___closed__12);
v___x_61_ = lean_alloc_ctor(0, 4, sizeof(size_t)*1);
lean_ctor_set(v___x_61_, 0, v___x_60_);
lean_ctor_set(v___x_61_, 1, v___x_59_);
lean_ctor_set(v___x_61_, 2, v___x_57_);
lean_ctor_set(v___x_61_, 3, v___x_57_);
lean_ctor_set_usize(v___x_61_, 4, v___x_56_);
return v___x_61_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__14(void){
_start:
{
lean_object* v___x_62_; uint64_t v___x_63_; lean_object* v___x_64_; 
v___x_62_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__13, &l_Lake_Toml_loadToml___closed__13_once, _init_l_Lake_Toml_loadToml___closed__13);
v___x_63_ = 0ULL;
v___x_64_ = lean_alloc_ctor(0, 1, 8);
lean_ctor_set(v___x_64_, 0, v___x_62_);
lean_ctor_set_uint64(v___x_64_, sizeof(void*)*1, v___x_63_);
return v___x_64_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__15(void){
_start:
{
lean_object* v___x_65_; 
v___x_65_ = l_Lean_PersistentHashMap_mkEmptyEntriesArray___redArg();
return v___x_65_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__16(void){
_start:
{
lean_object* v___x_66_; lean_object* v___x_67_; 
v___x_66_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__15, &l_Lake_Toml_loadToml___closed__15_once, _init_l_Lake_Toml_loadToml___closed__15);
v___x_67_ = lean_alloc_ctor(0, 1, 0);
lean_ctor_set(v___x_67_, 0, v___x_66_);
return v___x_67_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__17(void){
_start:
{
lean_object* v___x_68_; lean_object* v___x_69_; 
v___x_68_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__16, &l_Lake_Toml_loadToml___closed__16_once, _init_l_Lake_Toml_loadToml___closed__16);
v___x_69_ = lean_alloc_ctor(0, 2, 0);
lean_ctor_set(v___x_69_, 0, v___x_68_);
lean_ctor_set(v___x_69_, 1, v___x_68_);
return v___x_69_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__18(void){
_start:
{
lean_object* v___x_70_; lean_object* v___x_71_; lean_object* v___x_72_; 
v___x_70_ = l_Lean_NameSet_empty;
v___x_71_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__13, &l_Lake_Toml_loadToml___closed__13_once, _init_l_Lake_Toml_loadToml___closed__13);
v___x_72_ = lean_alloc_ctor(0, 3, 0);
lean_ctor_set(v___x_72_, 0, v___x_71_);
lean_ctor_set(v___x_72_, 1, v___x_71_);
lean_ctor_set(v___x_72_, 2, v___x_70_);
return v___x_72_;
}
}
static uint8_t _init_l_Lake_Toml_loadToml___closed__20(void){
_start:
{
lean_object* v___x_75_; lean_object* v___x_76_; uint8_t v___x_77_; 
v___x_75_ = l_Lean_diagnostics;
v___x_76_ = l_Lean_Options_empty;
v___x_77_ = l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__0(v___x_76_, v___x_75_);
return v___x_77_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__21(void){
_start:
{
lean_object* v___x_78_; lean_object* v___x_79_; lean_object* v___x_80_; 
v___x_78_ = l_Lean_maxRecDepth;
v___x_79_ = l_Lean_Options_empty;
v___x_80_ = l_Lean_Option_get___at___00Lake_Toml_loadToml_spec__1(v___x_79_, v___x_78_);
return v___x_80_;
}
}
static lean_object* _init_l_Lake_Toml_loadToml___closed__23(void){
_start:
{
lean_object* v___x_82_; lean_object* v___x_83_; 
v___x_82_ = ((lean_object*)(l_Lake_Toml_loadToml___closed__22));
v___x_83_ = l_Lean_stringToMessageData(v___x_82_);
return v___x_83_;
}
}
LEAN_EXPORT lean_object* l_Lake_Toml_loadToml(lean_object* v_ictx_84_){
_start:
{
lean_object* v___x_86_; uint32_t v___x_87_; lean_object* v___x_88_; 
v___x_86_ = lean_unsigned_to_nat(0u);
v___x_87_ = 0;
v___x_88_ = l_Lean_mkEmptyEnvironment(v___x_87_);
if (lean_obj_tag(v___x_88_) == 0)
{
lean_object* v_a_89_; lean_object* v___x_91_; uint8_t v_isShared_92_; uint8_t v_isSharedCheck_199_; 
v_a_89_ = lean_ctor_get(v___x_88_, 0);
v_isSharedCheck_199_ = !lean_is_exclusive(v___x_88_);
if (v_isSharedCheck_199_ == 0)
{
v___x_91_ = v___x_88_;
v_isShared_92_ = v_isSharedCheck_199_;
goto v_resetjp_90_;
}
else
{
lean_inc(v_a_89_);
lean_dec(v___x_88_);
v___x_91_ = lean_box(0);
v_isShared_92_ = v_isSharedCheck_199_;
goto v_resetjp_90_;
}
v_resetjp_90_:
{
lean_object* v___x_93_; lean_object* v_fn_94_; lean_object* v_inputString_95_; lean_object* v_fileName_96_; lean_object* v_fileMap_97_; lean_object* v___x_98_; lean_object* v___x_99_; lean_object* v___x_100_; lean_object* v___x_101_; lean_object* v___x_102_; lean_object* v___x_103_; lean_object* v___x_104_; lean_object* v_errorMsg_105_; 
v___x_93_ = l_Lake_Toml_toml;
v_fn_94_ = lean_ctor_get(v___x_93_, 1);
v_inputString_95_ = lean_ctor_get(v_ictx_84_, 0);
v_fileName_96_ = lean_ctor_get(v_ictx_84_, 1);
v_fileMap_97_ = lean_ctor_get(v_ictx_84_, 2);
v___x_98_ = l_Lean_Options_empty;
v___x_99_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__0, &l_Lake_Toml_loadToml___closed__0_once, _init_l_Lake_Toml_loadToml___closed__0);
v___x_100_ = lean_box(0);
lean_inc(v_a_89_);
v___x_101_ = lean_alloc_ctor(0, 4, 0);
lean_ctor_set(v___x_101_, 0, v_a_89_);
lean_ctor_set(v___x_101_, 1, v___x_98_);
lean_ctor_set(v___x_101_, 2, v___x_99_);
lean_ctor_set(v___x_101_, 3, v___x_100_);
v___x_102_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__1, &l_Lake_Toml_loadToml___closed__1_once, _init_l_Lake_Toml_loadToml___closed__1);
v___x_103_ = l_Lean_Parser_mkParserState(v_inputString_95_);
lean_inc_ref(v_ictx_84_);
lean_inc_ref(v_fn_94_);
v___x_104_ = l_Lean_Parser_ParserFn_run(v_fn_94_, v_ictx_84_, v___x_101_, v___x_102_, v___x_103_);
v_errorMsg_105_ = lean_ctor_get(v___x_104_, 4);
lean_inc(v_errorMsg_105_);
if (lean_obj_tag(v_errorMsg_105_) == 1)
{
lean_object* v_val_106_; lean_object* v___x_107_; lean_object* v___x_108_; lean_object* v___x_109_; lean_object* v___x_111_; 
lean_dec(v_a_89_);
v_val_106_ = lean_ctor_get(v_errorMsg_105_, 0);
lean_inc(v_val_106_);
lean_dec_ref_known(v_errorMsg_105_, 1);
v___x_107_ = l_Lake_mkParserErrorMessage(v_ictx_84_, v___x_104_, v_val_106_);
lean_dec_ref(v___x_104_);
v___x_108_ = l_Lean_MessageLog_empty;
v___x_109_ = l_Lean_MessageLog_add(v___x_107_, v___x_108_);
if (v_isShared_92_ == 0)
{
lean_ctor_set_tag(v___x_91_, 1);
lean_ctor_set(v___x_91_, 0, v___x_109_);
v___x_111_ = v___x_91_;
goto v_reusejp_110_;
}
else
{
lean_object* v_reuseFailAlloc_112_; 
v_reuseFailAlloc_112_ = lean_alloc_ctor(1, 1, 0);
lean_ctor_set(v_reuseFailAlloc_112_, 0, v___x_109_);
v___x_111_ = v_reuseFailAlloc_112_;
goto v_reusejp_110_;
}
v_reusejp_110_:
{
return v___x_111_;
}
}
else
{
lean_object* v_stxStack_113_; lean_object* v_pos_114_; uint8_t v___x_115_; 
lean_dec(v_errorMsg_105_);
v_stxStack_113_ = lean_ctor_get(v___x_104_, 0);
lean_inc_ref(v_stxStack_113_);
v_pos_114_ = lean_ctor_get(v___x_104_, 2);
lean_inc(v_pos_114_);
v___x_115_ = l_Lean_Parser_InputContext_atEnd(v_ictx_84_, v_pos_114_);
lean_dec(v_pos_114_);
if (v___x_115_ == 0)
{
lean_object* v___x_116_; lean_object* v___x_117_; lean_object* v___x_118_; lean_object* v___x_119_; lean_object* v___x_121_; 
lean_dec_ref(v_stxStack_113_);
lean_dec(v_a_89_);
v___x_116_ = ((lean_object*)(l_Lake_Toml_loadToml___closed__5));
v___x_117_ = l_Lake_mkParserErrorMessage(v_ictx_84_, v___x_104_, v___x_116_);
lean_dec_ref(v___x_104_);
v___x_118_ = l_Lean_MessageLog_empty;
v___x_119_ = l_Lean_MessageLog_add(v___x_117_, v___x_118_);
if (v_isShared_92_ == 0)
{
lean_ctor_set_tag(v___x_91_, 1);
lean_ctor_set(v___x_91_, 0, v___x_119_);
v___x_121_ = v___x_91_;
goto v_reusejp_120_;
}
else
{
lean_object* v_reuseFailAlloc_122_; 
v_reuseFailAlloc_122_ = lean_alloc_ctor(1, 1, 0);
lean_ctor_set(v_reuseFailAlloc_122_, 0, v___x_119_);
v___x_121_ = v_reuseFailAlloc_122_;
goto v_reusejp_120_;
}
v_reusejp_120_:
{
return v___x_121_;
}
}
else
{
lean_object* v___x_123_; lean_object* v___x_124_; lean_object* v___x_125_; lean_object* v___x_126_; lean_object* v___x_127_; uint8_t v___x_128_; lean_object* v___x_129_; lean_object* v___x_130_; lean_object* v___x_131_; lean_object* v___x_132_; lean_object* v___x_133_; lean_object* v___x_134_; lean_object* v___x_135_; lean_object* v___x_136_; lean_object* v___x_137_; lean_object* v___x_138_; lean_object* v___x_139_; lean_object* v___x_140_; lean_object* v___x_141_; lean_object* v___x_142_; uint8_t v___x_143_; lean_object* v___y_145_; lean_object* v___x_175_; uint8_t v___y_177_; lean_object* v_env_197_; uint8_t v___x_198_; 
lean_dec_ref(v___x_104_);
lean_del_object(v___x_91_);
v___x_123_ = l_Lean_Parser_SyntaxStack_back(v_stxStack_113_);
lean_dec_ref(v_stxStack_113_);
v___x_124_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__6, &l_Lake_Toml_loadToml___closed__6_once, _init_l_Lake_Toml_loadToml___closed__6);
v___x_125_ = l_Lean_firstFrontendMacroScope;
v___x_126_ = lean_box(0);
v___x_127_ = lean_box(0);
v___x_128_ = 0;
v___x_129_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__7, &l_Lake_Toml_loadToml___closed__7_once, _init_l_Lake_Toml_loadToml___closed__7);
v___x_130_ = ((lean_object*)(l_Lake_Toml_loadToml___closed__10));
v___x_131_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__11, &l_Lake_Toml_loadToml___closed__11_once, _init_l_Lake_Toml_loadToml___closed__11);
v___x_132_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__13, &l_Lake_Toml_loadToml___closed__13_once, _init_l_Lake_Toml_loadToml___closed__13);
v___x_133_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__14, &l_Lake_Toml_loadToml___closed__14_once, _init_l_Lake_Toml_loadToml___closed__14);
v___x_134_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__16, &l_Lake_Toml_loadToml___closed__16_once, _init_l_Lake_Toml_loadToml___closed__16);
v___x_135_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__17, &l_Lake_Toml_loadToml___closed__17_once, _init_l_Lake_Toml_loadToml___closed__17);
v___x_136_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__18, &l_Lake_Toml_loadToml___closed__18_once, _init_l_Lake_Toml_loadToml___closed__18);
v___x_137_ = lean_alloc_ctor(0, 3, 1);
lean_ctor_set(v___x_137_, 0, v___x_134_);
lean_ctor_set(v___x_137_, 1, v___x_134_);
lean_ctor_set(v___x_137_, 2, v___x_132_);
lean_ctor_set_uint8(v___x_137_, sizeof(void*)*3, v___x_115_);
v___x_138_ = ((lean_object*)(l_Lake_Toml_loadToml___closed__19));
v___x_139_ = lean_alloc_ctor(0, 9, 0);
lean_ctor_set(v___x_139_, 0, v_a_89_);
lean_ctor_set(v___x_139_, 1, v___x_129_);
lean_ctor_set(v___x_139_, 2, v___x_130_);
lean_ctor_set(v___x_139_, 3, v___x_131_);
lean_ctor_set(v___x_139_, 4, v___x_133_);
lean_ctor_set(v___x_139_, 5, v___x_135_);
lean_ctor_set(v___x_139_, 6, v___x_136_);
lean_ctor_set(v___x_139_, 7, v___x_137_);
lean_ctor_set(v___x_139_, 8, v___x_138_);
v___x_140_ = lean_st_mk_ref(v___x_139_);
v___x_141_ = l_Lean_inheritedTraceOptions;
v___x_142_ = lean_st_ref_get(v___x_141_);
v___x_143_ = lean_uint8_once(&l_Lake_Toml_loadToml___closed__20, &l_Lake_Toml_loadToml___closed__20_once, _init_l_Lake_Toml_loadToml___closed__20);
v___x_175_ = lean_st_ref_get(v___x_140_);
v_env_197_ = lean_ctor_get(v___x_175_, 0);
lean_inc_ref(v_env_197_);
lean_dec(v___x_175_);
v___x_198_ = l_Lean_Kernel_isDiagnosticsEnabled(v_env_197_);
lean_dec_ref(v_env_197_);
if (v___x_143_ == 0)
{
if (v___x_198_ == 0)
{
v___y_177_ = v___x_115_;
goto v___jp_176_;
}
else
{
v___y_177_ = v___x_143_;
goto v___jp_176_;
}
}
else
{
v___y_177_ = v___x_198_;
goto v___jp_176_;
}
v___jp_144_:
{
lean_object* v___x_146_; lean_object* v___x_147_; lean_object* v___x_148_; lean_object* v___x_149_; 
v___x_146_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__21, &l_Lake_Toml_loadToml___closed__21_once, _init_l_Lake_Toml_loadToml___closed__21);
lean_inc_ref(v_fileMap_97_);
lean_inc_ref(v_fileName_96_);
v___x_147_ = lean_alloc_ctor(0, 12, 0);
lean_ctor_set(v___x_147_, 0, v_fileName_96_);
lean_ctor_set(v___x_147_, 1, v_fileMap_97_);
lean_ctor_set(v___x_147_, 2, v___x_98_);
lean_ctor_set(v___x_147_, 3, v___x_146_);
lean_ctor_set(v___x_147_, 4, v___x_99_);
lean_ctor_set(v___x_147_, 5, v___x_100_);
lean_ctor_set(v___x_147_, 6, v___x_86_);
lean_ctor_set(v___x_147_, 7, v___x_124_);
lean_ctor_set(v___x_147_, 8, v___x_99_);
lean_ctor_set(v___x_147_, 9, v___x_125_);
lean_ctor_set(v___x_147_, 10, v___x_126_);
lean_ctor_set(v___x_147_, 11, v___x_142_);
v___x_148_ = lean_alloc_ctor(0, 3, 2);
lean_ctor_set(v___x_148_, 0, v___x_147_);
lean_ctor_set(v___x_148_, 1, v___x_86_);
lean_ctor_set(v___x_148_, 2, v___x_127_);
lean_ctor_set_uint8(v___x_148_, sizeof(void*)*3, v___x_143_);
lean_ctor_set_uint8(v___x_148_, sizeof(void*)*3 + 1, v___x_128_);
v___x_149_ = l_Lake_Toml_elabToml(v___x_123_, v___x_148_, v___y_145_);
lean_dec(v___y_145_);
lean_dec_ref_known(v___x_148_, 3);
if (lean_obj_tag(v___x_149_) == 0)
{
lean_object* v_a_150_; lean_object* v___x_152_; uint8_t v_isShared_153_; uint8_t v_isSharedCheck_163_; 
lean_dec_ref(v_ictx_84_);
v_a_150_ = lean_ctor_get(v___x_149_, 0);
v_isSharedCheck_163_ = !lean_is_exclusive(v___x_149_);
if (v_isSharedCheck_163_ == 0)
{
v___x_152_ = v___x_149_;
v_isShared_153_ = v_isSharedCheck_163_;
goto v_resetjp_151_;
}
else
{
lean_inc(v_a_150_);
lean_dec(v___x_149_);
v___x_152_ = lean_box(0);
v_isShared_153_ = v_isSharedCheck_163_;
goto v_resetjp_151_;
}
v_resetjp_151_:
{
lean_object* v___x_154_; lean_object* v_messages_155_; uint8_t v___x_156_; 
v___x_154_ = lean_st_ref_get(v___x_140_);
lean_dec(v___x_140_);
v_messages_155_ = lean_ctor_get(v___x_154_, 6);
lean_inc_ref(v_messages_155_);
lean_dec(v___x_154_);
v___x_156_ = l_Lean_MessageLog_hasErrors(v_messages_155_);
if (v___x_156_ == 0)
{
lean_object* v___x_158_; 
lean_dec_ref(v_messages_155_);
if (v_isShared_153_ == 0)
{
v___x_158_ = v___x_152_;
goto v_reusejp_157_;
}
else
{
lean_object* v_reuseFailAlloc_159_; 
v_reuseFailAlloc_159_ = lean_alloc_ctor(0, 1, 0);
lean_ctor_set(v_reuseFailAlloc_159_, 0, v_a_150_);
v___x_158_ = v_reuseFailAlloc_159_;
goto v_reusejp_157_;
}
v_reusejp_157_:
{
return v___x_158_;
}
}
else
{
lean_object* v___x_161_; 
lean_dec(v_a_150_);
if (v_isShared_153_ == 0)
{
lean_ctor_set_tag(v___x_152_, 1);
lean_ctor_set(v___x_152_, 0, v_messages_155_);
v___x_161_ = v___x_152_;
goto v_reusejp_160_;
}
else
{
lean_object* v_reuseFailAlloc_162_; 
v_reuseFailAlloc_162_ = lean_alloc_ctor(1, 1, 0);
lean_ctor_set(v_reuseFailAlloc_162_, 0, v_messages_155_);
v___x_161_ = v_reuseFailAlloc_162_;
goto v_reusejp_160_;
}
v_reusejp_160_:
{
return v___x_161_;
}
}
}
}
else
{
lean_object* v_a_164_; lean_object* v___x_166_; uint8_t v_isShared_167_; uint8_t v_isSharedCheck_174_; 
lean_dec(v___x_140_);
v_a_164_ = lean_ctor_get(v___x_149_, 0);
v_isSharedCheck_174_ = !lean_is_exclusive(v___x_149_);
if (v_isSharedCheck_174_ == 0)
{
v___x_166_ = v___x_149_;
v_isShared_167_ = v_isSharedCheck_174_;
goto v_resetjp_165_;
}
else
{
lean_inc(v_a_164_);
lean_dec(v___x_149_);
v___x_166_ = lean_box(0);
v_isShared_167_ = v_isSharedCheck_174_;
goto v_resetjp_165_;
}
v_resetjp_165_:
{
lean_object* v___x_168_; lean_object* v___x_169_; lean_object* v___x_170_; lean_object* v___x_172_; 
v___x_168_ = l_Lake_mkExceptionMessage(v_ictx_84_, v_a_164_);
v___x_169_ = l_Lean_MessageLog_empty;
v___x_170_ = l_Lean_MessageLog_add(v___x_168_, v___x_169_);
if (v_isShared_167_ == 0)
{
lean_ctor_set(v___x_166_, 0, v___x_170_);
v___x_172_ = v___x_166_;
goto v_reusejp_171_;
}
else
{
lean_object* v_reuseFailAlloc_173_; 
v_reuseFailAlloc_173_ = lean_alloc_ctor(1, 1, 0);
lean_ctor_set(v_reuseFailAlloc_173_, 0, v___x_170_);
v___x_172_ = v_reuseFailAlloc_173_;
goto v_reusejp_171_;
}
v_reusejp_171_:
{
return v___x_172_;
}
}
}
}
v___jp_176_:
{
if (v___y_177_ == 0)
{
lean_object* v___x_178_; lean_object* v_env_179_; lean_object* v_nextMacroScope_180_; lean_object* v_ngen_181_; lean_object* v_auxDeclNGen_182_; lean_object* v_traceState_183_; lean_object* v_messages_184_; lean_object* v_infoState_185_; lean_object* v_snapshotTasks_186_; lean_object* v___x_188_; uint8_t v_isShared_189_; uint8_t v_isSharedCheck_195_; 
v___x_178_ = lean_st_ref_take(v___x_140_);
v_env_179_ = lean_ctor_get(v___x_178_, 0);
v_nextMacroScope_180_ = lean_ctor_get(v___x_178_, 1);
v_ngen_181_ = lean_ctor_get(v___x_178_, 2);
v_auxDeclNGen_182_ = lean_ctor_get(v___x_178_, 3);
v_traceState_183_ = lean_ctor_get(v___x_178_, 4);
v_messages_184_ = lean_ctor_get(v___x_178_, 6);
v_infoState_185_ = lean_ctor_get(v___x_178_, 7);
v_snapshotTasks_186_ = lean_ctor_get(v___x_178_, 8);
v_isSharedCheck_195_ = !lean_is_exclusive(v___x_178_);
if (v_isSharedCheck_195_ == 0)
{
lean_object* v_unused_196_; 
v_unused_196_ = lean_ctor_get(v___x_178_, 5);
lean_dec(v_unused_196_);
v___x_188_ = v___x_178_;
v_isShared_189_ = v_isSharedCheck_195_;
goto v_resetjp_187_;
}
else
{
lean_inc(v_snapshotTasks_186_);
lean_inc(v_infoState_185_);
lean_inc(v_messages_184_);
lean_inc(v_traceState_183_);
lean_inc(v_auxDeclNGen_182_);
lean_inc(v_ngen_181_);
lean_inc(v_nextMacroScope_180_);
lean_inc(v_env_179_);
lean_dec(v___x_178_);
v___x_188_ = lean_box(0);
v_isShared_189_ = v_isSharedCheck_195_;
goto v_resetjp_187_;
}
v_resetjp_187_:
{
lean_object* v___x_190_; lean_object* v___x_192_; 
v___x_190_ = l_Lean_Kernel_enableDiag(v_env_179_, v___x_143_);
if (v_isShared_189_ == 0)
{
lean_ctor_set(v___x_188_, 5, v___x_135_);
lean_ctor_set(v___x_188_, 0, v___x_190_);
v___x_192_ = v___x_188_;
goto v_reusejp_191_;
}
else
{
lean_object* v_reuseFailAlloc_194_; 
v_reuseFailAlloc_194_ = lean_alloc_ctor(0, 9, 0);
lean_ctor_set(v_reuseFailAlloc_194_, 0, v___x_190_);
lean_ctor_set(v_reuseFailAlloc_194_, 1, v_nextMacroScope_180_);
lean_ctor_set(v_reuseFailAlloc_194_, 2, v_ngen_181_);
lean_ctor_set(v_reuseFailAlloc_194_, 3, v_auxDeclNGen_182_);
lean_ctor_set(v_reuseFailAlloc_194_, 4, v_traceState_183_);
lean_ctor_set(v_reuseFailAlloc_194_, 5, v___x_135_);
lean_ctor_set(v_reuseFailAlloc_194_, 6, v_messages_184_);
lean_ctor_set(v_reuseFailAlloc_194_, 7, v_infoState_185_);
lean_ctor_set(v_reuseFailAlloc_194_, 8, v_snapshotTasks_186_);
v___x_192_ = v_reuseFailAlloc_194_;
goto v_reusejp_191_;
}
v_reusejp_191_:
{
lean_object* v___x_193_; 
v___x_193_ = lean_st_ref_put(v___x_140_, v___x_192_);
lean_inc(v___x_140_);
v___y_145_ = v___x_140_;
goto v___jp_144_;
}
}
}
else
{
lean_inc(v___x_140_);
v___y_145_ = v___x_140_;
goto v___jp_144_;
}
}
}
}
}
}
else
{
lean_object* v_a_200_; lean_object* v___x_202_; uint8_t v_isShared_203_; uint8_t v_isSharedCheck_216_; 
v_a_200_ = lean_ctor_get(v___x_88_, 0);
v_isSharedCheck_216_ = !lean_is_exclusive(v___x_88_);
if (v_isSharedCheck_216_ == 0)
{
v___x_202_ = v___x_88_;
v_isShared_203_ = v_isSharedCheck_216_;
goto v_resetjp_201_;
}
else
{
lean_inc(v_a_200_);
lean_dec(v___x_88_);
v___x_202_ = lean_box(0);
v_isShared_203_ = v_isSharedCheck_216_;
goto v_resetjp_201_;
}
v_resetjp_201_:
{
lean_object* v___x_204_; lean_object* v___x_205_; lean_object* v___x_206_; lean_object* v___x_207_; lean_object* v___x_208_; uint8_t v___x_209_; lean_object* v___x_210_; lean_object* v___x_211_; lean_object* v___x_212_; lean_object* v___x_214_; 
v___x_204_ = lean_obj_once(&l_Lake_Toml_loadToml___closed__23, &l_Lake_Toml_loadToml___closed__23_once, _init_l_Lake_Toml_loadToml___closed__23);
v___x_205_ = lean_io_error_to_string(v_a_200_);
v___x_206_ = lean_alloc_ctor(3, 1, 0);
lean_ctor_set(v___x_206_, 0, v___x_205_);
v___x_207_ = l_Lean_MessageData_ofFormat(v___x_206_);
v___x_208_ = lean_alloc_ctor(7, 2, 0);
lean_ctor_set(v___x_208_, 0, v___x_204_);
lean_ctor_set(v___x_208_, 1, v___x_207_);
v___x_209_ = 2;
v___x_210_ = l_Lake_mkMessageNoPos(v_ictx_84_, v___x_208_, v___x_209_);
v___x_211_ = l_Lean_MessageLog_empty;
v___x_212_ = l_Lean_MessageLog_add(v___x_210_, v___x_211_);
if (v_isShared_203_ == 0)
{
lean_ctor_set(v___x_202_, 0, v___x_212_);
v___x_214_ = v___x_202_;
goto v_reusejp_213_;
}
else
{
lean_object* v_reuseFailAlloc_215_; 
v_reuseFailAlloc_215_ = lean_alloc_ctor(1, 1, 0);
lean_ctor_set(v_reuseFailAlloc_215_, 0, v___x_212_);
v___x_214_ = v_reuseFailAlloc_215_;
goto v_reusejp_213_;
}
v_reusejp_213_:
{
return v___x_214_;
}
}
}
}
}
LEAN_EXPORT lean_object* l_Lake_Toml_loadToml___boxed(lean_object* v_ictx_217_, lean_object* v_a_218_){
_start:
{
lean_object* v_res_219_; 
v_res_219_ = l_Lake_Toml_loadToml(v_ictx_217_);
return v_res_219_;
}
}
lean_object* runtime_initialize_Lean_Parser_Types(uint8_t builtin);
lean_object* runtime_initialize_Lake_Toml_Data_Value(uint8_t builtin);
lean_object* runtime_initialize_Lake_Toml_Elab(uint8_t builtin);
lean_object* runtime_initialize_Lake_Util_Message(uint8_t builtin);
lean_object* runtime_initialize_Std_Do(uint8_t builtin);
void lean_initialize();
static bool _G_runtime_initialized = false;
LEAN_EXPORT lean_object* runtime_initialize_Lake_Toml_Load(uint8_t builtin) {
lean_object * res;
if (_G_runtime_initialized) return lean_io_result_mk_ok(lean_box(0));
_G_runtime_initialized = true;
lean_initialize();
res = runtime_initialize_Lean_Parser_Types(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = runtime_initialize_Lake_Toml_Data_Value(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = runtime_initialize_Lake_Toml_Elab(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = runtime_initialize_Lake_Util_Message(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = runtime_initialize_Std_Do(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
return lean_io_result_mk_ok(lean_box(0));
}
static bool _G_meta_initialized = false;
LEAN_EXPORT lean_object* meta_initialize_Lake_Toml_Load(uint8_t builtin) {
lean_object * res;
if (_G_meta_initialized) return lean_io_result_mk_ok(lean_box(0));
_G_meta_initialized = true;
return lean_io_result_mk_ok(lean_box(0));
}
lean_object* initialize_Lean_Parser_Types(uint8_t builtin);
lean_object* initialize_Lake_Toml_Data_Value(uint8_t builtin);
lean_object* initialize_Lake_Toml_Elab(uint8_t builtin);
lean_object* initialize_Lake_Util_Message(uint8_t builtin);
lean_object* initialize_Std_Do(uint8_t builtin);
static bool _G_initialized = false;
LEAN_EXPORT lean_object* initialize_Lake_Toml_Load(uint8_t builtin) {
lean_object * res;
if (_G_initialized) return lean_io_result_mk_ok(lean_box(0));
_G_initialized = true;
res = initialize_Lean_Parser_Types(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = initialize_Lake_Toml_Data_Value(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = initialize_Lake_Toml_Elab(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = initialize_Lake_Util_Message(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = initialize_Std_Do(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = runtime_initialize_Lake_Toml_Load(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
res = meta_initialize_Lake_Toml_Load(builtin);
if (lean_io_result_is_error(res)) return res;
lean_dec_ref(res);
return initialize_Lake_Toml_Load(builtin);
}
#ifdef __cplusplus
}
#endif
